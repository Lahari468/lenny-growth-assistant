...
2. assigns source identifiers such as `[S1]` and `[S2]`;
3. constructs a grounded context block;
4. calls the configured chat provider;
5. returns answer text plus source metadata;
6. sanitizes invalid citation IDs;
7. logs operational metadata without logging raw prompt, transcript, or answer content.
